    ...
    <>
      <div className={styles.musicCollection}>
        ...
        <>
                ...
                <div className={styles.musicBox} key={id}>
                  <div className={styles.musicBoxImageDiv}>
                    <Image
                      ...
                      onClick={() => {
                        musicCtx.updateSongIndex(id);
                        musicCtx.playSong({
                          musicDp: songData.image
                            ? songData.image[songData.image.length - 1].url
                            : "/assets/no-image.png",
                          id: songData.id,
                          audioUrl:
                          ...
                          title: songData.name,
                          subtitle: songData.artists.primary.length > 0 &&
                          songData.artists.primary.map(artist => artist.name).join(", ").substring(0, 40),
                        });
                      }}
                    />
                    {musicCtx.activeSong &&
                    musicCtx.activeSong.id === songData.id &&
                    musicCtx.isPlaying ? (
                      <HiPause className={styles.playButtonIcon} />
                    ) : (
                      <AiFillPlayCircle className={styles.playButtonIcon} />
                    )}
                    {musicCtx.activeSong &&
                      musicCtx.activeSong.id === songData.id &&
                      musicCtx.isPlaying && (
                  ...
                  <div className={styles.musicTitleSubtitle}>
                    ...
                    >
                      {songData.name.length > 30
                        ? songData.name.slice(0, 30) + "..."
                        : songData.name}
                    </div>
                    <div className={styles.musicSubtitle}>
                      {/* {songData.artists.primary.length > 40 ? songData.primaryArtists.slice(0, 40)+"...":songData.primaryArtists} */}
                      {songData.artists.primary.length > 0 &&
          ...
          <button onClick={loadMore} className={styles.loadMoreSongsButton}>
            {loadMoreText}
          </button>
        </>
        {/* </Swiper> */}
